...
def track(booking_id):
    ...
    return render_template('tracking.html', booking=b, room=room)
...
def login_required(role=None):
    def decorator(fn):
        ...
        def wrapper(*args, **kwargs):
            ...
            if not user: return redirect(url_for('admin_login'))
            if role and user.get('role') != role: return ('Forbidden', 403)
...
def admin_start(booking_id):
    b = get_booking(booking_id)
    if not b: return ('Not found', 404)
    if start_simulation_for_booking(socketio, booking_id, b['pickup_lat'], b['pickup_lng'], b['dropoff_lat'], b['dropoff_lng']):
        set_booking_status(booking_id, 'in_progress'); flash('เริ่มจำลองการเดินทางแล้ว', 'success')
    else:
...
def admin_stop(booking_id):
    if stop_simulation_for_booking(booking_id):
        set_booking_status(booking_id, 'completed'); flash('หยุดการจำลองแล้ว', 'info')
    return redirect(url_for('admin_dashboard'))
